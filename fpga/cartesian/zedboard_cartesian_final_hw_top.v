...
module zedboard_cartesian_final_hw_top (
    input  wire       gclk_i,
...
(* MARK_DEBUG = "TRUE", KEEP = "TRUE" *) wire signed [5:0] dac_real_cart_w;
(* MARK_DEBUG = "TRUE", KEEP = "TRUE" *) wire signed [5:0] dac_imag_cart_w;
...
assign tx_real_o = dac_real_cart_w;
assign tx_imag_o = dac_imag_cart_w;
...
always @(posedge core_clk_w or negedge rstn_core_w) begin
    if (!rstn_core_w) begin
        ...
        if (start_tx_w)
            tx_activity_seen_r <= 1'b0;
        else if (system_busy_w && ((dac_real_cart_w != 6'sd0) || (dac_imag_cart_w != 6'sd0)))
...
CSS_Transmitter_Top u_css_tx (
    ...
    .DAC_RE_cart(dac_real_cart_w),
    .DAC_IM_cart(dac_imag_cart_w),
